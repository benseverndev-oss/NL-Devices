...
| `spike/fixtures/sensor_node/` | **Create** | Captured `ato build` output (`sensor_node.kicad_pcb`, unrouted) + hand-authored parser samples (`freerouting.ok.txt`, `freerouting.bad.txt`, `drc.json`, `gerbers.txt` — `.txt` not `.log`, since `spike/.gitignore` ignores `*.log`). The offline test inputs. |
